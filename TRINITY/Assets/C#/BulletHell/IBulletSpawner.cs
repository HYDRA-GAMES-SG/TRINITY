...
[RequireComponent(typeof(UBulletPool))]
public class IBulletSpawner : MonoBehaviour
{
    [HideInInspector]
    public IEnemyController EnemyController;

    [Header("Spawner Settings")] 
    private UBulletPool BulletPool;
    ...
    protected virtual void Start()
    {
        if (BulletPool == null)
        {
            BulletPool = gameObject.GetComponent<UBulletPool>();
        }

        if (EnemyController == null)
        {
            EnemyController = transform.parent.GetComponent<IEnemyController>();

            if (EnemyController == null)
            {
                Debug.LogError("No Enemy Controller on Bullet Spawner: " + transform.root.name);
            }
    ...
    protected virtual IEnumerator Spawn()
    {
        ...
        List<ABullet> WaitingBullets = new List<ABullet>();
        ...
        for (int i = 0; i < NumRepeats; i++)
        {
            ...
            for (int j = 0; j < nBullets; j++)
            {
                ...
                if (spawnedObject.GetComponent<ABullet>() != null)
                {
                    ABullet spawnedBullet = spawnedObject.GetComponent<ABullet>();
            ...
            foreach (ABullet b in WaitingBullets)
            {
                b.EnableMotion();
            }
