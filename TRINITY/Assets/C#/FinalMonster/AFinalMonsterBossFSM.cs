...
public class AFinalMonsterBossFSM : MonoBehaviour
{
    [Tooltip("The state used to start the state machine.")]
    public FinalMonsterBossState InitialState;

    private readonly Dictionary<string, FinalMonsterBossState> states = new Dictionary<string, FinalMonsterBossState>();
    private Queue<FinalMonsterBossState> TransitionQueue = new Queue<FinalMonsterBossState>();

    public FinalMonsterBossState CurrentState { get; private set; }
    public FinalMonsterBossState PreviousState { get; private set; }

    public event Action<FinalMonsterBossState, FinalMonsterBossState> OnStateChange;
    public AFinalMonsterController FinalMonsterBossController;
    ...
    private void ProcessTransitions()
    {
        if (TransitionQueue.Count > 0)
        {
            ...
            {
                FinalMonsterBossState nextState = TransitionQueue.Dequeue();
    ...
    private void TransitionToState(FinalMonsterBossState nextState)
    {
    ...
    public FinalMonsterBossState GetState(string stateName)
    {
        states.TryGetValue(stateName, out FinalMonsterBossState state);
        return state;
    }

    public T GetState<T>() where T : FinalMonsterBossState
    {
    ...
    {
        FinalMonsterBossState state = GetState(stateName);
        if (state != null)
        {
    ...
    public void EnqueueTransition<T>() where T : FinalMonsterBossState
    {
        FinalMonsterBossState state = GetState<T>();
        if (state != null)
        {
            TransitionQueue.Enqueue(state);
        }
    ...
    {
        FinalMonsterBossState[] statesArray = GetComponents<FinalMonsterBossState>();
