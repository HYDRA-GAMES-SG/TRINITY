...
public class APenguinBossFSM : MonoBehaviour
{
    [Tooltip("The state used to start the state machine.")]
    public PenguinBossState InitialState;

    private readonly Dictionary<string, PenguinBossState> states = new Dictionary<string, PenguinBossState>();
    private Queue<PenguinBossState> TransitionQueue = new Queue<PenguinBossState>();

    public PenguinBossState CurrentState { get; private set; }
    public PenguinBossState PreviousState { get; private set; }

    public event Action<PenguinBossState, PenguinBossState> OnStateChange;
    public APenguinController PenguinBossController;
    ...
    private void ProcessTransitions()
    {
        if (TransitionQueue.Count > 0)
        {
            ...
            {
                PenguinBossState nextState = TransitionQueue.Dequeue();
    ...
    private void TransitionToState(PenguinBossState nextState)
    {
    ...
    public PenguinBossState GetState(string stateName)
    {
        states.TryGetValue(stateName, out PenguinBossState state);
        return state;
    }

    public T GetState<T>() where T : PenguinBossState
    {
    ...
    {
        PenguinBossState state = GetState(stateName);
        if (state != null)
        {
    ...
    public void EnqueueTransition<T>() where T : PenguinBossState
    {
        PenguinBossState state = GetState<T>();
        if (state != null)
        {
            TransitionQueue.Enqueue(state);
        }
    ...
    {
        PenguinBossState[] statesArray = GetComponents<PenguinBossState>();
